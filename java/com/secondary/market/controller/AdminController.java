...
import com.secondary.market.domain.*;
import com.secondary.market.service.*;
import com.secondary.market.util.GoodsGrid;
import com.secondary.market.util.OrdersGrid;
import com.secondary.market.util.PurseGrid;
...
public class AdminController {
	...
	public String index(HttpServletRequest request, Admin admins) {
		Admin myadmin = adminService.findAdmin(admins.getUserName(), admins.getPassword());
		if (myadmin != null) {
...
//	}
	...
	/**
	 * 从Excel文件导入用户数据
	 * @param file 上传的Excel文件(.xls/.xlsx格式)
	 * @return 返回导入结果,包含:
	 *         success: 是否成功
	 *         message: 错误信息(如果失败)
	 *         data: 成功导入的用户数量(如果成功)
